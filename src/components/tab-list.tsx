...
import { X, FileText, Pencil, Home, ExternalLink } from 'lucide-react';
...
export function TabList() {
  ...
  const openPreviewWindow = (slug: string) => {
    if (window.electron) {
      window.electron.openPreviewWindow(slug);
    } else {
      window.open(`/preview/${slug}`, '_blank', 'width=800,height=900');
    }
  };
    ...
    >
      ...
      {tabs.map((tab) => {
        ...
        return (
          <DropdownMenu 
            key={tab.slug} 
            open={openMenuSlug === tab.slug} 
            onOpenChange={(open) => !open && setOpenMenuSlug(null)}
          >
            <div
            ...
            >
              {/* This hidden trigger acts as the anchor for the dropdown menu */}
              <DropdownMenuTrigger className="absolute bottom-0 left-0 w-full h-0 invisible pointer-events-none" />
              ...
              <button
                ...
                className={cn(
                  "relative p-0.5 rounded-md hover:bg-accent transition-all duration-200 z-50",
            ...
            <DropdownMenuContent align="start" className="w-48">
              <DropdownMenuItem onClick={() => openPreviewWindow(tab.slug)} className="gap-2">
                <ExternalLink className="h-3.5 w-3.5" />
                <span>Open Preview</span>
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => closeTab(tab.slug)} className="gap-2 text-destructive focus:text-destructive">
